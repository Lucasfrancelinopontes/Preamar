...
function pickSpecies(catalog, index) {
  if (!catalog.length) {
    throw new Error('Nenhuma especie foi retornada pela API.');
  }
...
async function main() {
  ...
  const municipio = municipios[0] || {};
      Promise.all(
        Array.from(mercadosPorTipo.entries()).map(async ([tipo, mercadoId]) => ({
          tipo,
          count: await PeixariaMercadoLinha.count({ where: { ID_mercado: mercadoId } })
        }))
      )
    id: Number(item.ID_especie ?? item.ID ?? item.id ?? item.id_especie),
    nome: String(item.nome_popular ?? item.Nome_popular ?? item.nome_cientifico ?? item.Nome_cientifico ?? 'Especie')
  ...
  if (!especiesCatalogo.length) {
    throw new Error('Nao foi possivel identificar IDs validos de especies.');
  }
  ...
  const perdasPorEspecie = makeSeries(COUNT, (index) => ({
    titulo: `Esppecie de perda ${index}`,
    linhas: [
      { causa: 'Deterioracao', estimativa: Number((index * 0.25).toFixed(2)), destino: `Destino A ${index}` },
      { causa: 'Falta de mercado', estimativa: Number((index * 0.2).toFixed(2)), destino: `Destino B ${index}` },
      { causa: 'Transporte', estimativa: Number((index * 0.15).toFixed(2)), destino: `Destino C ${index}` }
  ...
  createdId = createdId || createResponse?.data?.ID_peixaria;
  if (!createdId) {
    throw new Error('A API nao retornou ID_peixaria na criacao.');
  }

  const [
    despesasCount,
    fornecedoresCount,
    pescadoresLocalCount,
    pescadoresEntregaCount,
    especiesCadastradasCount,
    perdasCount,
    perdasPorEspecieCount,
    origemPescadoCount,
    relacoesCount,
    mercados,
    mercadoLinesCount
  ] = await Promise.all([
    PeixariaDespesa.count({ where: { ID_peixaria: createdId } }),
    PeixariaFornecedor.count({ where: { ID_peixaria: createdId } }),
    PeixariaPescadorFornecedor.count({ where: { ID_peixaria: createdId, tipo: 'LOCAL' } }),
    PeixariaPescadorFornecedor.count({ where: { ID_peixaria: createdId, tipo: 'ENTREGA' } }),
    PeixariaEspecieComercial.count({ where: { ID_peixaria: createdId } }),
    PeixariaPerda.count({ where: { ID_peixaria: createdId } }),
    PeixariaPerdaPorEspecie.count({ where: { ID_peixaria: createdId } }),
    PeixariaOrigemPescado.count({ where: { ID_peixaria: createdId } }),
    PeixariaRelacaoTrabalho.count({ where: { ID_peixaria: createdId } }),
    PeixariaMercado.findAll({
      where: { ID_peixaria: createdId },
      attributes: ['ID_mercado', 'tipo_mercado'],
      raw: true
    }),
    PeixariaMercadoLinha.count({
      include: [{
        model: PeixariaMercado,
        as: 'mercado',
        where: { ID_peixaria: createdId },
        attributes: []
      }]
  ...
  const mercadosCount = mercados.length;
  const mercadosPorTipo = new Map(
    mercados.map((mercado) => [String(mercado.tipo_mercado || '').toUpperCase(), mercado.ID_mercado])
  );
  ...
  const pescadores = pescadoresLocalCount + pescadoresEntregaCount;
  const mercadosValidados = mercadoLineCounts.length === 4 && mercadoLineCounts.every((item) => item.count === COUNT);
  ...
  const checks = {
    ...
    origem_pescado: origemPescadoCount === COUNT,
    relacoes_trabalho: relacoesCount === COUNT,
    mercados: mercadosCount === 4,
    mercado_lines: mercadoLinesCount === COUNT * 4,
    mercados_validados: mercadosValidados,
    pescadores_total: pescadores === COUNT * 2
  };
